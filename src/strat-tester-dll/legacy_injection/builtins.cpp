...
{
	ScrVar_t scrVar_t;
	UINT64* test = (UINT64*)((char*)OFF_ScrVarGlob + 128 + (scriptInst << 8));
	std::cout << std::endl;
	std::cout << "llpScrVarMemPool: " << test << std::endl;
	std::cout << "MEM_SCRVAR_SPACE: " << MEM_SCRVAR_SPACE(scriptInst) << std::endl;
	std::cout << (scriptInst ? MEM_SCRVAR_CSC_COUNT : MEM_SCRVAR_COUNT) << std::endl;
	std::cout << "scriptInst: " << scriptInst << std::endl;
	injector inj = injector();
	ReadProcessMemory(inj.pHandle, (LPCVOID)OFF_ScrVarGlob, &scrVar_t, sizeof(ScrVar_t), 0);
	std::cout << "ScrVarValue_t value.pad: " << scrVar_t.value.pad << std::endl;
